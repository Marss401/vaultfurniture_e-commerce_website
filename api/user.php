...
if(isset($_POST["trigger_delete"])){
    ...
    if($delete_query){
        ...
        echo json_encode(["error"=> true, "message"=> "Unable to delete User"]);
    }
}
...
if(isset($_POST["trigger_edit"])){
    ...
    if($save_query){
        echo json_encode(["error" => false, "message" => "User saved successfully"]);
    }
    else{
        echo json_encode(["error"=> true, "message"=> "Unable to save User"]);
    }
