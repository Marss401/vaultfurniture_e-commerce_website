...
<div class="bg-white p-6 mx-3 rounded-xl mb-6 border border-gray-200 shadow-sm">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500">Manage administrators and view account details</p>
        </div>
        ...
        <table class="min-w-full text-sm">
            ...
            <thead class="bg-gray-50 text-gray-600 uppercase text-xs tracking-wider">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left">Admin Details</th>
            ...
            <tbody class="divide-y divide-gray-100 text-gray-700">
                    <?php
                    $table = "users";
                    $find_query = $db->query("SELECT id, name, email, role FROM users WHERE role = 'admin' ORDER BY createdAt DESC");
                    if ($find_query->num_rows > 0) {
                        ...
                        while ($row = $find_query->fetch_assoc()) {
                            ...
                            // Use a HEREDOC to show admin details
                            echo <<<_HTML
                    <tr class="hover:bg-slate-100">
                        ...
                        <td class="text-center">
                            <div class="flex justify-center gap-2 w-full">
                                <button title="Edit" data-users='$data' class="editAdminBtn cursor-pointer py-1 px-3 bg-bluish text-primary text-sm rounded-md hover:bg-grey"><i class="fa-solid fa-file-pen"></i></button>
                                <button title="Delete" data-id='$id' data-page="admin.php" class="deleteBtn cursor-pointer py-1 px-3 bg-pink-500 text-white text-sm rounded-md hover:bg-pink-600"><i class="fa-solid fa-trash-can"></i></button>
                            </div>
                        </td>
                    </tr>
_HTML;
                        ...
                        echo '<tr><td colspan="5" class="text-center py-4">No Admin found</td></tr>';
                        exit;
                    }
                    ?>
    ...
    <dialog id="adminModal">
        <section
            ...
            <form id="adminForm" class="relative z-10 space-y-1.5 w-full max-w-md mx-auto bg-white p-6 rounded-lg shadow-lg">
                <h3 id="formTitle" class="text-xl font-semibold text-yellowish pb-2 mb-4 border-b border-slate-300">Add Admin</h3>
                ...
                <div class="relative">
                    ...
                    <input required type="text" name="fullname" id="fullname" placeholder="Abby Salako" class="py-2 px-4 rounded-md border border-slate-300 w-full text-slate-600 text-sm">
                </div>
                <div class="relative">
                    <label for="email" class="text-xs font-semibold text-slate-400">Email Address</label>
                    <input required type="email" name="email" id="email" placeholder="abbysal@gmail.com" class="py-2 px-4 rounded-md border border-slate-300 w-full text-slate-600 text-sm">
                </div>
